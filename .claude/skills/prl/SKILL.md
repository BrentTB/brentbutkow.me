...
description: >-
  ...
  and file stats (never the raw diff), then creates or updates the PR. Use when you want a
  quick PR and don't need the exhaustive structured description that the `pr` skill produces.
model: sonnet
---

# PR-light — quick PR into dev, low token cost
...
If `HEAD` is `dev` or `main`, stop: "On `dev`/`main` — check out a feature branch first."
...
obvious. No empty headings, no template scaffolding.
...
- **No PR:** `gh pr create --base dev --title "<title>" --body-file /tmp/pr-body.md`
