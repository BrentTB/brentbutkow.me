...
import { gravityWellBurn, gravityWellImpulse } from '../abilities/blackHole/gravity-pull'
import { getSupernovaState } from '../abilities/sun/supernova'
    ...
    switch (effect.kind) {
      ...
      case EffectKind.eventHorizon: {
        ...
        working = working.map((a) => {
          ...
          const moved = gravityWellImpulse(a, effect, dt)
          return damageAsteroid(
            moved,
            gravityWellBurn(effect.damage, dist, effect.radius, dt),
            true
          )
        })
        break
