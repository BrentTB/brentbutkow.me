...
// its compact tab labels, and the 'Location' scope naming.
describe('LocationSelector', () => {
  ...
  it('renders a tab per country and marks the active one', () => {
    ...
    for (const country of Object.values(RecallCountry)) {
      expect(screen.getByRole('button', { name: countryTabLabels[country] })).toBeTruthy()
    }
    expect(
      screen.getByRole('button', { name: countryTabLabels.us }).getAttribute('aria-pressed')
    ).toBe('true')
  })

  it('names the collapsed trigger with the location scope', () => {
    render(<LocationSelector value="us" collapsed onChange={() => {}} />)
    expect(screen.getByRole('button', { name: `Location: ${countryTabLabels.us}` })).toBeTruthy()
  })
