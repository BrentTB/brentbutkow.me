...
const topics = [
  ...
  { id: 1, label: 'undeclared · milk · wheat', topTerms: ['undeclared', 'milk', 'wheat'], size: 5 },
]

describe('Themes', () => {
  afterEach(cleanup)

  it('renders a row per theme and reports the chosen topic id on click', () => {
  ...
  it('clears the filter when the active theme is clicked again', () => {
    const onSelect = vi.fn()
    render(<Themes topics={topics} activeTopic="0" onSelect={onSelect} />)
