...
// magic + version + base + flags + uint32 payload length
export const CORE_HEADER_BYTES = MAGIC.length + 1 + 1 + 1 + LENGTH_BYTES
...
export function packHeader(header: ImageHeader): Uint8Array {
  ...
  bytes[o++] = VERSION
  bytes[o++] = header.base
  bytes[o++] = header.encrypted ? FLAG_ENCRYPTED : 0
...
export function parseCoreHeader(bytes: Uint8Array): CoreHeader | null {
  ...
  const encrypted = (bytes[o++] & FLAG_ENCRYPTED) === FLAG_ENCRYPTED
  const payloadByteLength =
    ((bytes[o] << 24) | (bytes[o + 1] << 16) | (bytes[o + 2] << 8) | bytes[o + 3]) >>> 0
  return { base, encrypted, payloadByteLength }
}
