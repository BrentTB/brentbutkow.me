# REPO_MAP

Auto-generated by `npm run repo-map`. Pre-commit hook keeps it fresh — do not edit by hand.

Format: `<file>  <exported symbols>`. Use this to locate code instead of grepping source.

```
src/
  App.tsx                                    App

src/components/CodeSection/
  CodeSection.tsx                            CodeBlock, CodeSection
  useCopyToClipboard.ts                      useCopyToClipboard

src/components/
  ModeToggle.tsx                             ModeToggle

src/components/PageFormatting/
  BackButton.tsx                             BackButton
  PageHeader.tsx                             PageHeader
  PageLayout.tsx                             PageLayout

src/components/ToggleableSection/
  ToggleableSection.tsx                      ToggleableSection

src/components/cards/
  ArticleOrLinkCard.tsx                      ArticleOrLinkCard
  DetailCard.tsx                             DetailCard

src/components/effects/
  WaterRipple.tsx                            WaterRipple

src/components/effects/shaders/
  waterRipple.frag.ts                        createFragmentShaderSource
  waterRipple.vert.ts                        vertexShaderSource

src/components/effects/
  useWaterRipple.ts                          useWaterRipple

src/components/footer/
  Footer.tsx                                 Footer

src/components/navbar/
  Navbar.tsx                                 Navbar
  useFocusTrap.ts                            useFocusTrap

src/components/utils/
  SafeLink.tsx                               SafeLink
  link-arrow.ts                              getLinkArrow

src/contexts/
  FunModeProvider.tsx                        FunModeProvider
  useFunMode.ts                              FunModeContextType, FunModeContext, useFunMode

src/data/
  data.types.ts                              HeroContent, HeroAction, Project, ExperienceProject, Experience, ContactInfo, ContactPlatform, Education, FunItem, Achievement
  jokes.ts                                   isJokeType, jokes
  jokes.types.ts                             JokeTypes, Joke, jokeTypeLabels

src/
  main.tsx                                   (no named exports)

src/modes/
  fun-mode.ts                                isFunModeEnabled, enableFunMode, disableFunMode

src/pages/Achievements/
  AchievementsPage.tsx                       AchievementsPage

src/pages/Achievements/components/
  AchievementCard.tsx                        AchievementCard

src/pages/Achievements/
  data.ts                                    achievements

src/pages/ContactMe/
  ContactMePage.tsx                          ContactMePage

src/pages/ContactMe/components/
  ContactCard.tsx                            ContactCard

src/pages/ContactMe/
  data.ts                                    contactPlatforms

src/pages/Education/
  EducationPage.tsx                          EducationPage

src/pages/Education/components/
  EducationCard.tsx                          EducationCard

src/pages/Education/
  data.ts                                    education

src/pages/Experience/
  ExperiencePage.tsx                         ExperiencePage

src/pages/Experience/components/
  ExperienceCard.tsx                         ExperienceCard

src/pages/Experience/
  data.ts                                    experience

src/pages/FunStuff/
  FunStuffPage.tsx                           FunStuffPage

src/pages/FunStuff/components/
  FunCard.tsx                                FunCard

src/pages/FunStuff/
  data.ts                                    funStuffSubRoutes, funStuff

src/pages/FunStuff/subpages/CourseProjects/
  CourseProjects.tsx                         CourseProjects
  data.ts                                    courseProjects

src/pages/FunStuff/subpages/Games/
  GamesPage.tsx                              GamesPage
  data.ts                                    gamesSubRoutes, games

src/pages/FunStuff/subpages/GulagSort/
  GulagSort.tsx                              GulagSort
  GulagSortVisualizer.tsx                    GulagSortVisualizer
  data.ts                                    gulagSortCode
  gulag-sort.ts                              GulagBlock, AnimationFrame, parseInput, generateRandomNumbers, isSorted, removeRemovedFromGulags, removeEmptyFinalGulag, moveBlockBetweenGulags, performGulagSort
  useGulagSort.ts                            useGulagSort

src/pages/FunStuff/subpages/NullSpace/
  NullSpace.tsx                              NullSpace

src/pages/FunStuff/subpages/NullSpace/components/ChangelogFilters/
  ChangelogFilters.tsx                       ChangelogFilters

src/pages/FunStuff/subpages/NullSpace/components/Development/
  DevConsole.tsx                             DevConsole

src/pages/FunStuff/subpages/NullSpace/components/GameHUD/
  Abilities.tsx                              Abilities
  GameHUD.tsx                                GameHUD
  LevelProgress.tsx                          LevelProgress
  RechargeRing.tsx                           RechargeRing
  SpaceMetalRail.tsx                         SpaceMetalRail
  TopBar.tsx                                 TopBar

src/pages/FunStuff/subpages/NullSpace/components/
  GameOverScreen.tsx                         GameOverScreen
  GameOverlay.tsx                            GameOverlay

src/pages/FunStuff/subpages/NullSpace/components/Icon/
  Icon.tsx                                   Icon, IconName
  gallery-entry.tsx                          (no named exports)

src/pages/FunStuff/subpages/NullSpace/components/PauseMenu/
  HelpScreen.tsx                             HelpScreen
  PauseMenu.tsx                              PauseMenu
  SettingsScreen.tsx                         SettingsScreen

src/pages/FunStuff/subpages/NullSpace/components/StartScreen/
  MenuScreen.tsx                             MenuScreen
  ShipSelectionScreen.tsx                    ShipSelectionScreen
  ShipSpritePreview.tsx                      ShipSpritePreview
  StatBar.tsx                                StatBar

src/pages/FunStuff/subpages/NullSpace/components/UpgradeScreen/
  UpgradeCard.tsx                            UpgradeCard
  UpgradeScreen.tsx                          UpgradeScreen
  WeaponDetail.tsx                           WeaponDetail
  WeaponsList.tsx                            WeaponsList
  weapons-list-order.ts                      orderWeaponsForShop

src/pages/FunStuff/subpages/NullSpace/components/
  WaveCompleteScreen.tsx                     WaveCompleteScreen

src/pages/FunStuff/subpages/NullSpace/
  data.ts                                    GAME_NAME, WORLD_SIZE, SHIP_DEFAULTS, ShipVariantStats, ShipVariantConfig, STAT_MAX, SHIELD_COOLDOWN, SHIP_ORDER, SHIP_VARIANTS, POWER_DEFAULTS, WEAPON_ORDER, ENEMY_STATS, CURRENCY_DROPS, CURRENCY_NAME, POWER_ORB, SPACE_METAL, WAVES_PER_LEVEL, SPAWN_DELAY, SPAWN_DISTANCE, SWARM_SPAWN_SPREAD, PROJECTILE_SPEED, PROJECTILE_LIFETIME, PROJECTILE_RADIUS, PARTICLE_DEFAULTS, ChangelogEntry, CHANGELOG, GAME_VERSION

src/pages/FunStuff/subpages/NullSpace/engine/abilities/
  ability-data.ts                            METEORITE_STRIKE, METEOR_STRIKE, BLACK_HOLE, ROCKET, SHIELD, SUN, HELPER, TELEKINESIS, SOLAR_FLARE
  ability-definition.ts                      AbilityActivation, AbilityDefinition, applyTierSum, applyCostReduction
  black-hole.ts                              blackHole
  helper.ts                                  helper
  hold-runtime.ts                            INACTIVE_HOLD_STATE, HoldBag, HoldAbilityConfig, runHoldAbility
  index.ts                                   ABILITY_DEFINITIONS, ABILITY_LIST, ABILITY_META, HOLD_ABILITIES, EFFECT_FACTORY, ALLY_FACTORY, WEAPON_UNLOCK_UPGRADE, ABILITY_UPGRADE_DEFINITIONS, createAbilities, applyTierSum, applyCostReduction, tryUseAbility, updateAbilityCooldowns, resolveAbilityInput, type AbilityResult
  meteor.ts                                  meteor
  meteorite.ts                               meteorite
  resolution.ts                              AbilityResult, tryUseAbility, updateAbilityCooldowns, resolveAbilityInput
  rocket.ts                                  rocket
  shield.ts                                  shield
  solar-flare.ts                             solarFlare
  sun.ts                                     sun
  telekinesis.ts                             telekinesis

src/pages/FunStuff/subpages/NullSpace/engine/entities/
  ally.ts                                    updateAllies
  enemy.ts                                   findNearestTarget, updateEnemyMovement, updateEnemyShooting
  entity-creator.ts                          uid, resetUid, createShip, createEnemy, createProjectile, createAlly, createParticle, spawnExplosionParticles, updateParticles, createAbilities
  ship.ts                                    applyDamageToShip, tickEscapeMode, updateShipPatrol, updateShipAttack

src/pages/FunStuff/subpages/NullSpace/engine/
  game-loop.ts                               createInitialState, moveToShipSelection, startGame, rollLevelUpWeaponOffers, startNextWave, applyUpgradeToState, finishUpgradeScreen, updateGameState

src/pages/FunStuff/subpages/NullSpace/engine/math/
  aoe.ts                                     AoeResult, damageEnemiesInRadiusFlat, damageEnemiesInRadius
  collision.ts                               checkCollision, distance, segmentIntersectsCircle
  homing.ts                                  homeTowardTarget
  random.ts                                  rng
  utils.ts                                   clamp

src/pages/FunStuff/subpages/NullSpace/engine/spaceMetalAbilities/
  escape-mode.ts                             ESCAPE_MODE, escapeDash
  index.ts                                   SPACE_METAL_ABILITIES, getSpaceMetalAbility, findSpaceMetalAbilityByKey, tryActivateSpaceMetalAbility, SpaceMetalAbilityKind, type SpaceMetalAbility
  shield-regen.ts                            shieldRegen
  space-metal-ability-definition.ts          SpaceMetalAbilityKind, SpaceMetalAbilityUIState, SpaceMetalAbility

src/pages/FunStuff/subpages/NullSpace/engine/systems/
  collectibles.ts                            spawnCollectiblesFromKills, updateCollectibles, tryCollectSpaceMetal
  combat.ts                                  updateProjectiles, resolveProjectileEnemyCollisions, resolveEnemyProjectileShipCollisions, resolveEnemyProjectileAllyCollisions, resolveEnemyShipCollisions, resolveEnemyAllyMeleeCollisions, resolveDeathEffects
  economy.ts                                 computeCurrencyFromKills
  effects.ts                                 EffectTickContext, EffectTickResult, updateActiveEffects, createMeteoriteEffect, createMeteorEffect, createBlackHoleEffect, createRocketEffect, createShieldEffect, applyShieldConstraints, createSunEffect
  spawner.ts                                 spawnPositionNearShip, processSpawnQueue

src/pages/FunStuff/subpages/NullSpace/engine/
  types.ts                                   Vec2, Entity, ShipKind, EscapeModePhase, EscapeModeState, Ship, EnemyKind, MovementBehavior, DeathBehavior, Enemy, ProjectileOwner, Projectile, AbilityKind, Ability, EffectKind, EffectBase, MeteorStrikeEffect, BlackHoleEffect, RocketEffect, ShieldEffect, SunEffect, ActiveEffect, CollectibleKind, Collectible, Ally, Particle, GamePhase, UpgradeCategory, UpgradeId, UpgradeTier, UpgradeDefinition, PlayerUpgrades, GameState, HoldRuntimeState, PlayerInput
  upgrades.ts                                UNLOCK_UPGRADE_IDS, UPGRADE_DEFINITIONS, isWeaponFullyMaxed, UPGRADE_CATEGORY_LABELS, createInitialUpgrades, canPurchaseUpgrade, purchaseUpgrade, applyUpgradesToAbilities, applyUpgradesToShip, applyUpgradesToPowerRegen, getLevel, isUpgradeWave, WEAPON_UNLOCK_UPGRADE

src/pages/FunStuff/subpages/NullSpace/engine/world/
  persistence.ts                             loadHighScore, saveHighScore, ChangelogCategory, ChangelogFilters, DEFAULT_CHANGELOG_FILTERS, CHANGELOG_CATEGORIES, loadChangelogFilters, saveChangelogFilters
  time.ts                                    MAX_DT, GameTime, createGameTime, tickGameTime, pauseGameTime, resumeGameTime, setGameSpeed
  waves.ts                                   getWave, getWaveDelay

src/pages/FunStuff/subpages/NullSpace/
  icon-names.ts                              IconName

src/pages/FunStuff/subpages/NullSpace/renderer/
  camera.ts                                  REFERENCE_VIEW, DEFAULT_GAME_ZOOM, Camera, createCamera, computeZoom, HUD_SCALE_MIN, HUD_SCALE_MAX, computeHudScale, isWithinView, updateCamera, centerCameraOn, worldToScreen, screenToWorld
  renderer.ts                                SHIP_SPRITE_KEY, renderFrame
  sprite-cache.ts                            SpriteCache, buildSpriteCache, getSpriteSize
  sprites.ts                                 SpriteData, SHIP_SPRITE, DRONE_SPRITE, TANK_SPRITE, PROJECTILE_SPRITE, METEOR_SPRITE, METEORITE_SPRITE, SHOOTER_SPRITE, ENEMY_PROJECTILE_SPRITE, SWARM_SPRITE, BOMBER_SPRITE, ROCKET_SPRITE, INTERCEPTOR_SPRITE, DREADNOUGHT_SPRITE, CARRIER_SPRITE, ALLY_SPRITE, SpriteKey, SPRITE_MAP
  starfield.ts                               Star, generateStarfield, renderStarfield

src/pages/FunStuff/subpages/NullSpace/
  useNullSpace.ts                            GameUIState, DevPatch, getUnlockedAbilitiesInOrder, abilityKindForHotkey, useNullSpace
  usePseudoFullscreenChrome.ts               usePseudoFullscreenChrome

src/pages/Home/
  HomePage.tsx                               HomePage

src/pages/Home/components/
  About.tsx                                  About
  CurrentWork.tsx                            CurrentWork
  Hero.tsx                                   Hero

src/pages/Home/
  data.ts                                    cvPath, cvDownloadEnabled, heroContent, aboutSectionEnabled, aboutParagraphs
  useJokes.ts                                ALL_CATEGORY, JokeCategory, useJokes

src/pages/NotFound/
  NotFoundPage.tsx                           NotFoundPage
  useParticleField.ts                        useParticleField

src/routes/
  Router.tsx                                 Router
  routes.config.tsx                          routePaths, funStuffRoutes, routes
  routes.types.ts                            AppRoute
  useDocumentTitle.ts                        useDocumentTitle
```
